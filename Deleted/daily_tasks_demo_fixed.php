...
$additionalJS = "
    <script>
        class DailyTasksManager {
            ...
            async loadTaskStats() {
                try {
                    const response = await fetch('../api/tasks/daily_simple.php');
            ...
            async loadTodayTasks() {
                ...
                try {
                    ...
                    if (data.success && data.data && data.data.today && data.data.today.tasks.length > 0) {
            ...
            async loadTeamTasks() {
                ...
                try {
                    const response = await fetch('../api/tasks/team.php');
                    const data = await response.json();
                    
                    loadingEl.style.display = 'none';
                    
                    if (data.status === 'success' && data.data && data.data.length > 0) {
                        contentEl.innerHTML = this.renderTasksTable(data.data, true);
                    } else {
